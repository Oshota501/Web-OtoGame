...
export default class DomEventAdder {
    ...
    public chartLoader: RandomChartLoader = new RandomChartLoader(80 * 2, 0.5, 30, 0, 0);

    constructor(player: MusicPlayer) {
        const start_button = document.getElementById("music-start");
        if (start_button) start_button.addEventListener("click", () => {
            setInterval(async () => {
        ...
        if (range) range.addEventListener("input", () => {
            this.chartLoader.step = Number((range as HTMLInputElement).value);
            if (this.chartLoader.step != 0) this.chartLoader.length = 80 * (1 / this.chartLoader.step);
        });

        const half = document.getElementById("chart-setting-number-of-half");
        if (half) half.addEventListener("input", () => {
            this.chartLoader.half = Number((half as HTMLInputElement).value);
        });

        const quanter = document.getElementById("chart-setting-number-of-quanter");
        if (quanter) quanter.addEventListener("input", () => {
            this.chartLoader.quarter = Number((quanter as HTMLInputElement).value);
        });

        const triplet = document.getElementById("chart-setting-number-of-triplet");
        if (triplet) triplet.addEventListener("input", () => {
            this.chartLoader.triplet = Number((triplet as HTMLInputElement).value);
        });
    }
}
